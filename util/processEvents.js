...
function unhandledRejection([ error, ...args], client){
  ...
  const time = parseDate(new Date(Date.now() + offset).toLocaleString('en-UK',{ timezone: client.config.timezone }).split(/:|\s|\//));

  if (!channel){
    return Promise.resolve(console.log(String(error.stack).grey));
  } else {
    // do nothing
  };
